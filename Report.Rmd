...
\newpage
# Data Table
The table below shows the data collected for each box including a link to the Google Images file of the original photo. This link will also provide the location, in a preview map format next to the image, if you should wish to know the location of a specific energy box.

```{r data table, echo=FALSE}
# Create an interactive data table using the DT package
all_data %>%
  select(Photo, Location, box_type, Google_Photos_Link) %>%
  mutate(Google_Photos_Link = paste0("[link](", Google_Photos_Link, ")")) %>%
  rename("Box Type" = box_type, "Google Photos Link" = Google_Photos_Link) %>%
